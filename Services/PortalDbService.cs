...
namespace OracleSqlPortal.Services
{
    ...
    public class PortalDbService
    {
        ...
        public List<QueryHistory> GetQueryHistoryForUser(string username, int count = 200)
        {
            try
            {
                using var conn = Open();
                using var cmd = new OracleCommand(
                    @"SELECT USERNAME,DISPLAY_NAME,ENVIRONMENT,SQL_TEXT,CLIENT_IP,
                     ROWS_AFFECTED,DURATION_MS,IS_ERROR,EXECUTED_AT
              FROM (SELECT * FROM QUERY_HISTORY WHERE UPPER(USERNAME)=UPPER(:u) ORDER BY ID DESC)
              WHERE ROWNUM<=:n", conn);
            ...
            {
                LogError(ex, nameof(GetQueryHistoryForUser));
        ...
        // ── SP execution result ───────────────────────────────────
        public class SpExecuteResult
        {
            public string ScalarOutput { get; set; } = "";   // OUT param key=value lines
            public string DbmsOutput   { get; set; } = "";   // DBMS_OUTPUT.PUT_LINE text
            public string Error        { get; set; } = "";
            public List<SpCursorResult> Cursors { get; set; } = new();
        }

        public class SpCursorResult
        {
            ...
            List<(string name, string type, string direction, string value)> parameters)
        {
            ...
            try
            {
                ...
                // ── Enable DBMS_OUTPUT ─────────────────────────────
                using (var enableCmd = new Oracle.ManagedDataAccess.Client.OracleCommand(
                    "BEGIN DBMS_OUTPUT.ENABLE(1000000); END;", conn))
                ...
                // ── Bind parameters with correct Oracle types ──────
                foreach (var (name, type, dir, val) in parameters)
                {
                    bool isOut   = dir.ToUpper() is "OUT" or "INOUT";
                    bool isIn    = dir.ToUpper() is "IN"  or "INOUT";
                    bool isCursor = type.ToUpper() is "REF CURSOR" or "SYS_REFCURSOR";

                    Oracle.ManagedDataAccess.Client.OracleParameter p;

                    if (isCursor)
                    {
                        p = cmd.Parameters.Add(name,
                            Oracle.ManagedDataAccess.Client.OracleDbType.RefCursor);
                        p.Direction = System.Data.ParameterDirection.Output;
                    }
                    else
                    {
                        var oraType = type.ToUpper() switch
                        {
                            "NUMBER" or "INTEGER" or "INT" or "FLOAT" or "DECIMAL"
                                => Oracle.ManagedDataAccess.Client.OracleDbType.Decimal,
                            "DATE"
                            ...
                            "BLOB"
                                => Oracle.ManagedDataAccess.Client.OracleDbType.Blob,
                            _ => Oracle.ManagedDataAccess.Client.OracleDbType.Varchar2
                        };

                        p = cmd.Parameters.Add(name, oraType);
                        p.Direction = dir.ToUpper() switch
                        {
                        ...
                        p.Size = 4000;
                        if (isIn && !string.IsNullOrEmpty(val))
                            p.Value = val;
                    }
                }
                ...
                // ── Collect scalar OUT params ──────────────────────
                var scalarSb = new System.Text.StringBuilder();
                foreach (Oracle.ManagedDataAccess.Client.OracleParameter p in cmd.Parameters)
                {
                    bool isOut = p.Direction is System.Data.ParameterDirection.Output
                                              or System.Data.ParameterDirection.InputOutput;
                    if (!isOut) continue;

                    if (p.OracleDbType == Oracle.ManagedDataAccess.Client.OracleDbType.RefCursor)
                    {
                        // ── Read cursor rows ───────────────────────
                        var cursorResult = new SpCursorResult { Name = p.ParameterName };
                        using var reader = ((Oracle.ManagedDataAccess.Types.OracleRefCursor)p.Value).GetDataReader();
                        for (int i = 0; i < reader.FieldCount; i++)
                            cursorResult.Columns.Add(reader.GetName(i));
                        while (reader.Read())
                        {
                            var row = new List<string>();
                            for (int i = 0; i < reader.FieldCount; i++)
                                row.Add(reader.IsDBNull(i) ? "" : reader.GetValue(i)?.ToString() ?? "");
                            cursorResult.Rows.Add(row);
                        }
                        result.Cursors.Add(cursorResult);
                    }
                    else
                    {
                        scalarSb.AppendLine($"{p.ParameterName} = {p.Value}");
                    }
                }
                result.ScalarOutput = scalarSb.ToString();

                // ── Drain DBMS_OUTPUT ──────────────────────────────
                var dbmsSb = new System.Text.StringBuilder();
                using var getLineCmd = new Oracle.ManagedDataAccess.Client.OracleCommand(
                    "BEGIN DBMS_OUTPUT.GET_LINE(:line, :status); END;", conn);
                var lineParam   = getLineCmd.Parameters.Add("line",   Oracle.ManagedDataAccess.Client.OracleDbType.Varchar2);
                lineParam.Direction = System.Data.ParameterDirection.Output;
                lineParam.Size  = 32767;
                var statusParam = getLineCmd.Parameters.Add("status", Oracle.ManagedDataAccess.Client.OracleDbType.Int32);
                ...
                while (true)
                {
                    ...
                    if (status != 0) break;   // 1 = no more lines
                    dbmsSb.AppendLine(lineParam.Value?.ToString() ?? "");
                }
                result.DbmsOutput = dbmsSb.ToString();
            }
            catch (Exception ex)
            {
